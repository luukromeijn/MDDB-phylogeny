...
def write_to_fasta(childs, records, add_seqs, new_records):
    ...
    for seq in add_seqs:
        new_sequence = new_records.seq_list[new_records.id_list.index(seq)]
        new_file.write(">" + seq + "\n" + new_sequence + "\n")
...
def update_tree():
    ...
    # for x in tqdm(range(100)):
        if unknown_records.seq_list[x] not in seq_records.seq_list:
            ...
            avg_smallest_dist += sorted_distances[0][1]
            best_dist = [sorted_distances[x][1] for x in range(len(sorted_distances)) if sorted_distances[x][1] <= 1.025*sorted_distances[0][1]]
            avg_best += sum(best_dist)/len(best_dist)
            avg_worst_dist += best_dist[-1]
            best_distances = [sorted_distances[x][0] for x in range(len(sorted_distances)) if sorted_distances[x][1] <= 1.025*sorted_distances[0][1]]
            avg_dist += len(best_distances)
            if len(best_distances):
                ancestor = ref_tree.common_ancestor(best_distances)
                if ancestor in anc_dict:
                    anc_dict[ancestor].append(unknown_records.id_list[x])
                else:
                    anc_dict[ancestor] = [unknown_records.id_list[x]]
        else:
            print('Zit er al in')
            # pass
    depth_anc = []
    for anc in anc_dict:
        path = ref_tree.get_path(anc)
        depth_anc.append((anc, len(path), anc_dict[anc]))
    sorted_paths = sorted(depth_anc, key = lambda i: i[1])[::-1]
    skipped = 0
    for ancs in tqdm(sorted_paths):
        outgroups, constraint = get_outgroups(ancs[0], ref_tree)
        Phylo.write(constraint, 'constraint.newick', 'newick')
        child_names = []
        if len(ancs[0].clades) == 0:
            child_names.append(ancs[0].name)
        else:
            find_names(ancs[0], child_names)
        child_names.extend(outgroups)
        avg_childs += len(child_names)
        if len(child_names) < 500:
            write_to_fasta(child_names, overall_records, ancs[2], unknown_records)
            create_subtree(ancs[0], outgroups)
        else:
