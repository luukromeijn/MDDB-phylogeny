...
# test_tree = Phylo.read('backbone_plus_Basidiomycota_Agaricomycetes_Gomphales.newick', 'newick')
...
def write_to_fasta2(childs, records, add_seqs, new_records):
    new_file = open('new_file.fasta', 'w')
    for child in childs:
        # try:
        sequence = records.seq_list[records.id_list.index(child)]
...
def update_tree():
    ...
    for ancs in tqdm(sorted_paths):
        ...
        child_names.extend(outgroups)
        write_to_fasta2(child_names, seq_records, anc_dict[ancs[0]], unknown_records)
